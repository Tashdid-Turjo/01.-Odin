...
5. dispatchEvent method -> Event constructor, dispatchEvent method.
// This method programmatically create and dispatch events using Event constructor and dispatchEvent() method.
// N.B: Events are generated by user actions such as mouse clicks and key presses. 
// In addition, events can be generated from code. Ex of it:
...
/*
For generating an event programmatically, the steps:
  i. crete a new Event object using 'Event constructor'.              -> new Event('type')
  ii. then trigger the event using 'element.dispatchEvent() method'.  -> element.dispatchEvent(event)
*/
...
// N.B: in this example, the button will be automatically triggered and the 
// popup msg will be shown(without clicking the button).
// N.B: another property is this: 'event.isTrusted' -> if the button is 
// clicked / event is generated by code, event.isTrusted is false, but if it's 
// from user action, then event.isTrusted property is true.
